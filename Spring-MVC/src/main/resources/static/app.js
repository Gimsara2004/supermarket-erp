...
// ---------- Login gate ----------
// NOTE: this only hides/shows the UI. The backend API endpoints themselves are
// not protected by any authentication - this project implements the login data
// structure and flow, not a full security layer (no Spring Security involved).
...
function showApp(user) {
  ...
  sessionInfo.textContent = `Logged in as ${user.fullName} (${user.role ? user.role.name : ''})`;
  initApp();
}
...
loginForm.addEventListener('submit', async (e) => {
  ...
  const res = await fetch('/api/auth/login', {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      username: loginUsernameField.value.trim(),
...
// On page load: if a session already exists (e.g. page refresh), skip straight to the app.
