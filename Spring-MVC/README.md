...
The app now opens on a login screen instead of straight into Products. On a brand-new database, `DataSeeder` (`config/DataSeeder.java`) automatically creates a default account on first startup so you're never locked out:

- **username:** `admin`
- **password:** `admin123`

Log in, then create your own users under the Users section, and feel free to delete/change the default account afterwards.

`POST /api/auth/login` checks the username/password against the `app_users` table (`AuthServiceImpl`) and returns the matching user (password never included in the response, thanks to `@JsonProperty(access = WRITE_ONLY)` on `User.password`) or a 401 with `InvalidCredentialsException` if the credentials are wrong or the account's `status` isn't `ACTIVE`.

**Important limitation, stated plainly:** this is UI-level gating only — `app.js` hides the app until login succeeds and remembers the session in `sessionStorage`. The backend API endpoints themselves are **not** protected by Spring Security or any token/session check; anyone who calls `/api/products` etc. directly still gets a response. Passwords are also stored as plain text, not hashed. This matches the assignment's scope (data structure + relationships from the ER diagram, plus a working login flow), not a production-grade security implementation. If real access control is ever needed, the next step would be adding Spring Security with hashed passwords and endpoint-level authorization.
